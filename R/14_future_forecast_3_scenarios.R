# ============================================================
# 14_future_forecast_3_scenarios.R - Phase 7gA:
# Post-2022 External Holdout and Short-Horizon True Future Forecasts
#
# Purpose:
#   1. Evaluate the model family on genuinely later data:
#      January 2023 through the latest observed CPI month.
#   2. Produce short-horizon true future forecasts beyond the latest
#      observed CPI for 1-, 3-, and 6-month windows.
#
...
future_horizons <- c(1L, 3L, 6L)
future_start_override <- as.yearmon("2026-07")

cat("=== Phase 7gA: Post-2022 External Holdout and 1/3/6-Month Future Forecasts ===\n\n")
...
# ============================================================
# STEP 5 - Short-horizon true future forecasts: 1, 3, and 6 months
# ============================================================

cat("Building short-horizon true future forecasts (1, 3, and 6 months)...\n")

latest_cpi_month <- max(dates)
future_start_month <- max(as.yearmon(latest_cpi_month) + 1 / 12,
                          future_start_override)

last_wti_mean_13 <- mean(tail(as.numeric(wti_wk_recent), 13L), na.rm = TRUE)

scenarios <- list(
  `Base: WTI flat` = rep(0, 4L),
  `Recent mean` = rep(last_wti_mean_13, 4L),
  `Upside oil shock` = rep(0.01, 4L),
  `Downside oil shock` = rep(-0.01, 4L)
)

get_month_wti4 <- function(month_ym, scenario_weekly) {
  m_start <- as.Date(month_ym)
  m_end <- as.Date(month_ym + 1 / 12) - 1L
  wti_dates <- as.Date(index(wti_wk_recent))
  in_month <- which(wti_dates >= m_start & wti_dates <= m_end)
  if (length(in_month) >= 4L) {
    return(as.numeric(tail(wti_wk_recent[in_month], 4L)))
  }
  scenario_weekly
}
...
generate_future_forecasts <- function(horizon_h, scenario_name, scenario_weekly) {
  future_months <- seq(future_start_month, by = 1 / 12, length.out = horizon_h)
  future_dates <- as.Date(future_months)

  fut_X4 <- do.call(rbind, lapply(seq_along(future_months), function(h) {
    get_month_wti4(future_months[h], scenario_weekly)
  }))
  colnames(fut_X4) <- colnames(X4)
  ...
  ar_pred <- tryCatch(
    as.numeric(forecast(fit_ar_full, h = horizon_h,
                        xreg = matrix(fut_x_monthly, ncol = 1))$mean),
    error = function(e) rep(NA_real_, horizon_h)
  )

  ne_pred <- tryCatch(
    if (is.null(fit_ne_full)) rep(NA_real_, horizon_h) else
  ...
  nb_pred <- tryCatch(
    if (is.null(fit_nb_full)) rep(NA_real_, horizon_h) else
  ...
  um_pred <- tryCatch(
    if (is.null(fit_um_full)) rep(NA_real_, horizon_h) else
  ...
  lasso_pred <- if (!is.null(fit_lasso_full)) {
    ...
    rep(NA_real_, horizon_h)
  }

  xgb_pred <- rep(NA_real_, horizon_h)
  if (!is.null(fit_xgb_full)) {
    y_aug <- c(y_all, rep(NA_real_, horizon_h))
  ...
  clm_pred <- rep(NA_real_, horizon_h)
  ...
  data.frame(
    Forecast_window = paste0(horizon_h, "-month"),
...
future_rows <- list()
row_id <- 1L
for (horizon_h in future_horizons) {
  for (sc_name in names(scenarios)) {
    future_rows[[row_id]] <- generate_future_forecasts(
      horizon_h = horizon_h,
      scenario_name = sc_name,
      scenario_weekly = scenarios[[sc_name]]
    )
    row_id <- row_id + 1L
  }
}
...
num_cols <- setdiff(names(future_tbl), c("Forecast_window", "Scenario", "Forecast_month"))
future_tbl[num_cols] <- lapply(future_tbl[num_cols], function(z) round(z, 6))

write.csv(future_tbl, "output/tables/future_forecast_136_scenarios.csv",
          row.names = FALSE)
saveRDS(future_tbl, "data/processed/future_forecast_136_scenarios.rds")

future_md <- c(
  "# Short-Horizon True Future Forecasts",
  "",
  sprintf("Latest observed CPI log-change month: %s.", format(latest_cpi_month, "%Y-%m")),
  sprintf("Future windows start at %s and cover 1-, 3-, and 6-month horizons.",
          format(as.Date(future_start_month), "%Y-%m")),
  "",
...
for (i in seq_len(nrow(future_tbl))) {
  future_md <- c(future_md, sprintf(
    "| %s | %s | %s | %.5f | %.5f | %.5f | %.5f | %.5f | %.5f | %.5f |",
    future_tbl$Forecast_window[i], future_tbl$Scenario[i], future_tbl$Forecast_month[i],
...
writeLines(future_md, "output/tables/future_forecast_136_scenarios.md")

render_png("output/figures/39_future_forecast_136_windows.png",
           width = 12, height = 4.5, {
  plot_windows <- c("1-month", "3-month", "6-month")
  plot_scenarios <- c("Base: WTI flat", "Recent mean", "Upside oil shock", "Downside oil shock")
  cols <- c("steelblue", "grey35", "tomato", "forestgreen")
  names(cols) <- plot_scenarios
  par(mfrow = c(1, 3), mar = c(4.2, 4.2, 3.4, 1))
  for (window_name in plot_windows) {
    plot_tbl <- future_tbl[
      future_tbl$Forecast_window == window_name &
        future_tbl$Scenario %in% plot_scenarios, ,
      drop = FALSE
    ]
    plot_dates <- as.Date(paste0(plot_tbl$Forecast_month, "-01"))
    y_range <- range(plot_tbl$`MIDAS nbeta`, na.rm = TRUE)
    base_idx <- plot_tbl$Scenario == "Base: WTI flat"
    plot(plot_dates[base_idx], plot_tbl$`MIDAS nbeta`[base_idx],
         type = "l", lwd = 2, col = cols["Base: WTI flat"], ylim = y_range,
         xlab = "", ylab = "Forecast CPI log-change",
         main = paste(window_name, "window"))
    for (sc_name in setdiff(plot_scenarios, "Base: WTI flat")) {
      sc_idx <- plot_tbl$Scenario == sc_name
      lines(plot_dates[sc_idx], plot_tbl$`MIDAS nbeta`[sc_idx],
            lwd = 2, col = cols[sc_name], lty = 1)
    }
    abline(h = 0, col = "grey60", lty = 2)
  }
  legend("bottom", inset = -0.45, horiz = TRUE, bty = "n", lwd = 2,
         col = cols, legend = names(cols), xpd = NA)
  mtext("Displayed model: MIDAS nbeta; table includes all models and all short-horizon windows.",
        side = 1, line = 6.2, outer = FALSE, cex = 0.8)
})
...
cat("\n=== Short-horizon future forecasts saved ===\n")
cat("Latest observed CPI month:", format(latest_cpi_month, "%Y-%m"), "\n")
cat("Future forecast start:    ", format(as.Date(future_start_month), "%Y-%m"), "\n")
cat("Forecast windows:         ", paste(future_horizons, collapse = ", "),
    "months\n\n")
...
cat("  output/tables/future_forecast_136_scenarios.csv\n")
cat("  output/tables/future_forecast_136_scenarios.md\n")
cat("Saved figures:\n")
...
cat("  output/figures/39_future_forecast_136_windows.png\n")
cat("Saved data:\n")
cat("  data/processed/post2022_external_forecasts.rds\n")
cat("  data/processed/updated_oos_2015_2026_forecasts.rds\n")
cat("  data/processed/future_forecast_136_scenarios.rds\n\n")

cat("Next thesis step:\n")
cat("  Use this script for the main thesis future-forecast section (1, 3, and\n")
cat("  6 months), then use R/15_future_scenarios_12_months.R for the 12-month\n")
cat("  appendix scenario extension.\n")
